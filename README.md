...
├── macOS defaults     ├── OpenCode agent/plugin
├── GPG setup          ├── Beyond Compare 5
├── LaunchAgents       ├── FileZilla
└── AI tool templates  └── VS Code / iTerm2
```
...
15 個設定項目透過 symlink 在本地與 iCloud 同步，每 6 小時自動備份，每 2 小時進行健康檢查。
...
| Beyond Compare 5 | `~/Library/Application Support/Beyond Compare 5` | dir |
| Codex skills (agents) | `~/.agents` | dir |
| FileZilla | `~/.config/filezilla` | dir |
...
OpenCode 設定檔位於 `~/.config/opencode/opencode.json`，由 chezmoi 直接管理（靜態 JSON）。

**Model 設定**：常用 model 已預設在檔案中（Antigravity、Gemini CLI 系列）。

調整 model：

```bash
chezmoi edit ~/.config/opencode/opencode.json   # 編輯設定檔
chezmoi apply                                    # 套用變更
```
...
設定檔位於 `~/.codex/config.toml`，透過 `modify_` script 與本機現有值合併（而非覆寫）。需要 `AZURE_OPENAI_ENDPOINT` 環境變數（定義在 `~/.secrets`）。

具名 profiles（Codex CLI 0.144+ 要求的格式）另外產生於 `~/.codex/codex.config.toml`。
