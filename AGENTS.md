...
| `dot_tool-versions` | `~/.tool-versions` | asdf 語言版本（asdf 專用，`version_manager=mise` 時僅為相容保留） |
...
| 檔案 | 目標路徑 | 說明 |
|------|----------|------|
| `private_dot_codex/modify_private_config.toml.tmpl` | `~/.codex/config.toml` | Codex CLI 設定（Azure OpenAI profiles，`modify_` 合併，現有值優先） |
| `private_dot_codex/codex.config.toml.tmpl` | `~/.codex/codex.config.toml` | Codex 具名 profiles（CLI 0.144+ 格式） |
| `private_dot_codex/hooks.json` | `~/.codex/hooks.json` | Codex hooks 設定 |
| `private_dot_codex-mcp/config.toml.tmpl` | `~/.codex-mcp/config.toml` | Codex MCP wrapper 設定（不含 MCP servers，避免 model detection 逾時） |
| `private_dot_codex-mcp/codex.config.toml.tmpl` | `~/.codex-mcp/codex.config.toml` | Codex MCP wrapper 具名 profiles |
| `private_dot_config/opencode/` | `~/.config/opencode/` | OpenCode 設定 |
| `Library/Application Support/Claude/modify_private_claude_desktop_config.json` | `~/Library/Application Support/Claude/` | Claude Desktop 設定（`modify_` 合併） |

`.chezmoitemplates/`（`codex-base.toml`、`codex-profiles.toml`）：Codex 相關 template 共用片段，本身不產生目標檔案。

### Docker

| 檔案 | 目標路徑 | 說明 |
|------|----------|------|
| `private_dot_docker/modify_private_config.json` | `~/.docker/config.json` | Docker 設定（`modify_` 合併，保留 `docker login` 寫入的 `auths`） |
...
| `run_onchange_after_08-setup-mcp.sh.tmpl` | after | MCP 設定同步至 `~/.claude.json` |
...
**同步項目：** Claude agents/skills/hooks/HUD、CLAUDE.md、.mcp.json、Codex skills、Claude Code Router、OpenCode agent/plugin/superpowers、Beyond Compare 5、FileZilla、VS Code extensions、iTerm2 preferences
...
- chezmoi source 直接指向本 repo（`install.sh` 以 `--source` 指定；本機亦可於 `~/.config/chezmoi/chezmoi.toml` 設定 `sourceDir`），`apply` 直接讀 workspace，改完即生效，無獨立副本 drift 問題
